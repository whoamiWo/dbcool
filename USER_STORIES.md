...
- P0 partial:14(需补齐验收标准)
- P0 not-started:1(US-504 应用切换)
- **结论:若以"done"为 MVP 达标线,MVP 完成率 54.5%,存在重大终止风险。**

### 7.4 P0 完全 done 清单(18 个)

US-001(建表), US-002(11种字段类型), US-005(修改表), US-201(表格视图), US-204(看板), US-205(详情), US-206(列设置), US-301(用户管理), US-302(角色分配), US-304(记录级权限), US-305(操作权限), US-401(工作流CRUD), US-403(审批会签), US-405(条件分支), US-407(执行历史), US-501(登录), US-502(改密), US-503(站内信)

### 7.5 P0 partial 清单(14 个,需补齐)

US-003(字段约束-主键/唯一/默认值), US-004(关联字段-前端), US-101(拖拽建表单), US-102(字段属性面板), US-103(显隐规则), US-104(校验规则), US-105(预览模式), US-106(提交后动作), US-202(筛选), US-203(排序), US-303(字段级权限), US-402(节点拖拽), US-404(邮件通知), US-406(测试工作流)
