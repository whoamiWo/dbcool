...
- **P0 完全 done:33 / 33 = 100%**(2026-09-17 更新:US-003、US-102~106、US-202、US-203、US-303、US-402、US-406、US-004、US-101、US-404、US-504 已达 done)
- P0 partial:0
- P0 not-started:0
- **结论:所有 P0(33 个)已全部达 done —— MVP 达标线 100%。✅**

### 7.4 P0 完全 done 清单(33 个)
...
**US-303(字段级权限-FIELD 策略 hidden 配置)**, **US-004(关联字段-选关联表+显示对方字段)**,
**US-101(拖拽建表单-5节点 ReactFlow)**, **US-404(邮件通知-SMTP 配置表单)**, **US-504(应用切换-user_tenant+UI)**

### 7.5 P0 partial 清单(0 个)
...
>
> **US-004 已达 done(2026-09-17 复核判定,非新增实现)**:初审曾判 partial(理由"关联字段前端不完整"),
> 复核确认**是真缺口** —— 原 `SchemaEditor` 字段表只有「字段名/类型/必填」,
> 选 belongsTo/hasMany 时**无任何入口设置 options.target**,
> 而后端 `RelationResolver.expandRelations` 依赖该字段展开(测试里甚至用空 target 占位)。
> 本次补齐:
> - 加载全部 collection 供「关联表」下拉;
> - `handleOptionChange` 设置 options.target(不覆盖已有字段);
> - 关联类型时额外渲染配置行 + `TargetFieldsPreview` 组件,选中目标表后实时展示其字段列表。
> `tsc -b` ✓、`vitest` 207/207 PASS、无回归。
>
> **US-101 已达 done(2026-09-17 复核判定,非新增实现)**:初审曾判 partial(理由"拖拽建表单不完整"),
> 复核属**误判** —— `WorkflowDesigner.tsx:316-328` 左侧面板遍历 `nodeKindMeta` 渲染 4 种节点
> (审批/通知/条件/HTTP),每个 `draggable` + `onDragStart`;`:207-223` `onDrop` 创建节点;
> `:365-368` 右侧 `NodeConfigEditor` 配置节点;`:225-256` 保存序列化 nodes/edges。
> 拖拽建表单链路完整闭环。
>
> **US-404 已达 done(2026-09-17 复核判定,非新增实现)**:初审曾判 partial(理由"无专门 SMTP 配置 UI"),
> 复核确认**是真缺口** —— `NotificationChannels.tsx` 原 EMAIL 类型仅靠自由 JSON 文本框填写 config,
> 无显式 SMTP 明细(主机/端口/用户名/密码/加密/发件人),用户需手写 JSON。
> 本次补齐:
> - `editing.type === 'EMAIL'` 时展示 SMTP 专用表单(host/port/username/password/encryption/from);
> - 表单值实时同步到 channel.config,保存后仍可手动编辑 JSON。
> `tsc -b` ✓、`vitest` 207/207 PASS。
